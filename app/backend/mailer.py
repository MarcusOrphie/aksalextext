...
def send_pixels_thanks(email: str, name: str = "", px: int = 0, total: int = 0):
    """Письмо покупателю пикселей после оплаты (Тусовка брендов / brands.aksalex.com)."""
    site = "https://brands.aksalex.com"
    brand = (name or "").strip()
    px_s = "{:,}".format(int(px or 0)).replace(",", " ")
    sum_s = "{:,}".format(int(total or 0)).replace(",", " ")
    head = ("Логотип «" + brand + "» размещён!") if brand else "Ваш логотип размещён!"
    html = f"""<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#faf5ec;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#faf5ec;"><tr><td align="center" style="padding:28px 16px;">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
<tr><td style="padding:6px 4px 18px;font-family:'Oswald',Arial,sans-serif;font-weight:700;text-transform:uppercase;letter-spacing:1px;font-size:20px;color:#151210;">
  <img src="https://aksalex.com/apple-touch-icon.png" width="34" height="34" alt="" style="vertical-align:middle;border-radius:50%;border:2px solid #151210;margin-right:9px;"> ТУСОВКА БРЕНДОВ</td></tr>
<tr><td style="background:#fffdf8;border:3px solid #151210;border-radius:18px;padding:34px 30px;box-shadow:8px 8px 0 #ff7f50;">
  <div style="font-family:'Oswald',Arial,sans-serif;font-weight:600;text-transform:uppercase;letter-spacing:2px;font-size:12px;color:#e85f2c;">Оплата прошла</div>
  <h1 style="margin:8px 0 12px;font-family:'Oswald',Arial,sans-serif;font-weight:700;text-transform:uppercase;font-size:26px;line-height:1.08;color:#151210;">{head}</h1>
  <p style="margin:0 0 16px;font-family:'Nunito',Arial,sans-serif;font-size:16px;line-height:1.6;color:#2c2621;font-weight:600;">Спасибо, что заняли место на Тусовке брендов! Ваш логотип теперь на общем холсте - его видят все, кто заходит на brands.aksalex.com, а клик по нему ведёт на ваш сайт.</p>
  <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 20px;"><tr>
    <td style="font-family:'Nunito',Arial,sans-serif;font-size:15px;color:#2c2621;font-weight:700;padding:6px 18px 6px 0;">Пикселей: <span style="color:#e85f2c;">{px_s}</span></td>
    <td style="font-family:'Nunito',Arial,sans-serif;font-size:15px;color:#2c2621;font-weight:700;padding:6px 0;">Оплачено: <span style="color:#e85f2c;">{sum_s} &#8381;</span></td>
  </tr></table>
  <table role="presentation" cellpadding="0" cellspacing="0"><tr><td style="border-radius:14px;background:#ff7f50;border:3px solid #151210;">
    <a href="{site}" style="display:inline-block;padding:14px 30px;font-family:'Oswald',Arial,sans-serif;font-weight:700;text-transform:uppercase;font-size:16px;letter-spacing:1px;color:#faf5ec;text-decoration:none;">Посмотреть на холсте →</a></td></tr></table>
  <p style="margin:20px 0 0;font-family:'Nunito',Arial,sans-serif;font-size:14px;line-height:1.55;color:#4a443d;font-weight:600;">Нужно поменять логотип или ссылку - просто ответьте на это письмо.</p>
</td></tr>
<tr><td style="padding:18px 6px;font-family:'Nunito',Arial,sans-serif;font-size:12px;color:#7b7168;font-weight:600;">
  Тусовка брендов · <a href="https://aksalex.com" style="color:#7b7168;text-decoration:underline;">aksalex.com</a><br>
  <span style="color:#9a8f83;">Аксенов Александр Андреевич · ИНН 773102096413</span></td></tr>
</table></td></tr></table></body></html>"""
